...
{
    public sealed class BleDeviceStub : IBleDevice
    {
        private static readonly ILog Log = LogManager.GetLogger<BleDeviceStub>();

        private readonly IUnityToNative unityToNative;

        public BleDeviceStub(string id)
        {
            ID = id;
            
            unityToNative = new EditorUnityToNative(new NativeToUnity());
        }
        ...
        {
            //Devices shutdown acknowledge receive, set device to disconenct so we dont have to wait for timeout before disconnect
