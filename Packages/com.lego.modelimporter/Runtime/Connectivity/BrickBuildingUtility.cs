...
namespace LEGOModelImporter
{
    public class BrickBuildingUtility
    {
        ...
        public static int IgnoreMask { get; } = ~LayerMask.GetMask(ConnectivityReceptorLayerName, ConnectivityConnectorLayerName);
        ...
        public static Collider[] ColliderBuffer { get; } = new Collider[512];
        public static RaycastHit[] RaycastBuffer { get; } = new RaycastHit[64];
        ...
        /// <param name="transformation">Transformation matrix you can use to specify the space you want the points in.</param>
        /// <returns></returns>
        ...
        public static Bounds ComputeBounds(Brick brick)
        {          
            // Get the bounding corners in world space
            var corners = GetBoundingCorners(brick.totalBounds, brick.transform.localToWorldMatrix);
        ...
        /// <returns></returns>
        public static Vector3 GetOffsetToGrid(Vector3 focusBrickPosition, ICollection<Brick> bricks, Ray ray, Vector3 pickupOffset, Matrix4x4 transformation)
        {
        ...
        public static void AlignBricks(Brick sourceBrick, ICollection<Brick> bricks, Bounds bounds, Vector3 pivot,
        ...
        public static HashSet<ConnectionField> Connect(Connection src, Connection dst, HashSet<Brick> onlyConnectTo = null, HashSet<Brick> ignoreForCollision = null)
        {
            ...
            if(!EditorApplication.isPlayingOrWillChangePlaymode)
            {
                ...
                foreach (var (c1, c2) in connections.planarFeatures)
                {
                    ...
                    if (c1.Field.insideDetail)
                    {
                        toRecord.Add(c1.Field.insideDetail.gameObject);
                    }
                    if (c2.Field.insideDetail)
                    {
                        toRecord.Add(c2.Field.insideDetail.gameObject);
                    }
                }
        ...
        /// <returns></returns>
        public static bool FindBestConnection(Vector3 pickupOffset, ICollection<Brick> selectedBricks, Ray ray, Matrix4x4 viewTransform, Brick[] bricksToCheck, Bounds bounds, out ConnectionResult[] result, int maxTries = DefaultMaxBricksToConsiderWhenFindingConnections)
        {
        ...
        public static void AlignTransformations(ICollection<Brick> selectedBricks, Vector3 pivot, Vector3 axis, float angle, Vector3 offset)
        {
        ...
        public static bool CheckConnectionCandidate(ConnectionField sourceField, ConnectionField destinationField, ICollection<Brick> selectedBricks, Vector3 pickupOffset, Ray ray, float maxSqrDistance, Bounds bounds, out ConnectionResult result)
        {
            oldPositions.Clear();
            oldRotations.Clear();
            ...
            if (destinationField is AxleField destinationAxle && sourceField is AxleField sourceAxle)
            {
                ...
                if (valid)
                {
                    ...
                    result.colliding = Colliding(selectedBricks, bounds, selectedBrick.gameObject.scene.GetPhysicsScene(), out _);
        ...
        internal static bool FindBestConnectionOnBrick(ICollection<Brick> selectedBricks, HashSet<ConnectionField> selectedFields, Vector3 pickupOffset, Brick brick, Ray ray, float maxSqrDistance, Bounds bounds, out ConnectionResult result)
        {
        ...
        public static bool CheckRejection(IEnumerable<Brick> bricks, out float overlapCompensation, Brick ignore = null)
        {
        ...
        public static bool Colliding(ICollection<Brick> bricks, Bounds bounds, PhysicsScene physicsScene, out int hits,  bool checkRejection = false)
        {
            ...
            foreach (var brick in bricks)
            {
                if (IsCollidingAtTransformation(brick, brick.transform.localToWorldMatrix, bricks))
                {
                    return true;
                }
            }
        ...
        private static void ResetTransformations(ICollection<Brick> bricks, List<Vector3> positions, List<Quaternion> rotations, Brick ignore = null)
        {
            var j = 0;
