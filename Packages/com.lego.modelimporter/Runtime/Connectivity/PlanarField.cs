...
namespace LEGOModelImporter
{
    ...
    public class PlanarField : ConnectionField
    {
        ...
        [HideInInspector] public InsideDetail insideDetail;
        ...
        /// <param name="updateDetail">Whether or not to update detail geometry (inside detail, knobs and tubes)</param>
        public void OnConnectionChanged(PlanarFeature connection, bool updateDetail = true)
        {            
        ...
        /// <param name="updateDetails">Whether or not to update knob and tube visibility</param>
        public void Connect(PlanarFeature src, PlanarFeature dst, bool updateDetails = true)
        {
            Connect(src.index, dst, updateDetails);
        }
        ...
        internal void Disconnect(int index, bool updateHiddenGeometry = true)
        {
            ...
            OnConnectionChanged(connections[index], updateHiddenGeometry);
            otherField.OnConnectionChanged(otherField.connections[indexOfConnection], updateHiddenGeometry);
        }

        internal void Disconnect(PlanarFeature connection, bool updateHiddenGeometry = true)
        {
            if(connection.index >= connections.Length)
            {
                return;
            }
            Disconnect(connection.index, updateHiddenGeometry);            
        }

        internal void Connect(int src, PlanarFeature dst, bool updateHiddenGeometry = true)
        {
            ...
            {
                entry.field.Disconnect(entry.indexOfConnection, updateHiddenGeometry);
            }

            if(dst == null)
            {
                connectedTo[src] = null;
                OnConnectionChanged(srcConnection, updateHiddenGeometry);
                return;
            }
            ...
            dst.Field.Connect(dstIndex, srcConnection, updateHiddenGeometry);

            // Call before updating geometry to make sure all data is up to date
            OnConnectionChanged(srcConnection, updateHiddenGeometry);
        }

        private static void Disconnect(ICollection<(PlanarFeature, PlanarFeature)> toBeDisconnected, bool updateHiddenGeometry = true)
        {
#if UNITY_EDITOR
            ...
            {
                c1.Field.Disconnect(c1, updateHiddenGeometry);
