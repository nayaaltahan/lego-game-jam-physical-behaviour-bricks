...
namespace LEGOModelImporter
{
    ...
    public class Brick : MonoBehaviour
    {
        ...
        public static Material TransparentMaterial => transparentMaterial;

        public void SetMaterial(bool ghosted, bool recordUndo = true)
        {
            if(ghosted && transparentMaterial == null)
            {
                transparentMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/LEGO_GhostedBrick.mat");;
            }
            ...
            foreach(var part in parts)
            {
                if(part.transform.childCount > 0)
                {
                    ...
                    {
                        material = Part.GetMaterial(part.materialIDs[0]);
                    }
                    ...
                    var detail = part.transform.Find("Detail");
                    if (detail)
                    {
                        var mrs = detail.GetComponentsInChildren<MeshRenderer>(true);
                        renderersToEdit.AddRange(mrs);
                    }
