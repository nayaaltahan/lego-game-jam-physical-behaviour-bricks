...
namespace LEGOModelImporter
{
    public class AssetVersionChecker
    {
        ...
        static readonly string dontAskUpdateToThisVersionPrefsKey = "com.lego.modelimporter.dontAskUpdateForVersion";
        ...
        private static bool UpdatePartGeometry(Part part)
        {
            ...
            if (partToInstantiate)
            {
                ...
                var detailToInstantiate = partToInstantiate.transform.Find("Detail");
                if (detailToInstantiate)
                {
                    Transform oldDetail = part.transform.Find("Detail");
                    if (oldDetail)
                    {
                    ...
                    var detail = UnityEngine.Object.Instantiate(detailToInstantiate);
                    detail.name = "Detail";
                    detail.parent = part.transform;
                    detail.localPosition = part.transform.localPosition;
                    detail.localRotation = part.transform.localRotation;

                    foreach (Transform detailTransform in detail)
                    {
                        var insideDetailComp = detailTransform.gameObject.AddComponent<InsideDetail>();
                        insideDetailComp.part = part;
                        part.insideDetails.Add(insideDetailComp);

                        var mr = detailTransform.GetComponent<MeshRenderer>();
                        mr.sharedMaterial = material;
                    }
                }
            }
                    ...
                    if(connectivity && connectivity.version != currentVersion)
                    {
                        ...
                        if (connectivityToInstantiate)
                        {
                            ...
                            foreach (var field in connectivityComp.planarFields)
                            {
                                ...
                                ModelImporter.MatchFieldWithInsideDetail(field, part.insideDetails);
                            }
                        }
                        else
                        {
