...
namespace LEGOModelImporter
{
    ...
    {
        public static readonly string designIdMappingPath = "Packages/com.lego.modelimporter/Data/designid.xml";
        public static readonly string legacyPartsPath = "Packages/com.lego.modelimporter/Data/LegacyParts.zip";
        public static readonly string newPartsPath = "Packages/com.lego.modelimporter/Data/NewParts.zip";
        public static readonly string commonPartsPath = "Packages/com.lego.modelimporter/Data/CommonParts.zip";
        ...
        static ZipFile legacyPartsZipFile;
        static ZipFile newPartsZipFile;
        static ZipFile commonPartsZipFile;
        ...
        {
            if (legacyPartsZipFile != null)
            {
                legacyPartsZipFile.Close();
                legacyPartsZipFile = null;
            }

            if (newPartsZipFile != null)
            {
                newPartsZipFile.Close();
                newPartsZipFile = null;
            }

            if (commonPartsZipFile != null)
            {
                commonPartsZipFile.Close();
                commonPartsZipFile = null;
            }
        ...
        static void OpenDB()
        {
            if (legacyPartsZipFile == null)
            {
                legacyPartsZipFile = new ZipFile(legacyPartsPath);
            }

            if (newPartsZipFile == null)
            {
                newPartsZipFile = new ZipFile(newPartsPath);
            }

            if (commonPartsZipFile == null)
            {
                commonPartsZipFile = new ZipFile(commonPartsPath);
            }
        ...
        public static List<string> GetPartList()
        {
            ...
            foreach (ZipEntry entry in newPartsZipFile)
            {
                if (entry.IsFile && entry.Name.StartsWith("Geometry/VX"))
                {
        ...
        {
            if (File.Exists(Path.Combine(geometryPath, newDir, lod0Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, newDir, lightmappedDir, lod0Dir, designID + ".fbx")))
            {
                return true;
            }

            if (File.Exists(Path.Combine(geometryPath, newDir, lod1Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, newDir, lightmappedDir, lod1Dir, designID + ".fbx")))
            {
                return true;
            }

            if (File.Exists(Path.Combine(geometryPath, newDir, lod2Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, newDir, lightmappedDir, lod2Dir, designID + ".fbx")))
            {
                return true;
            }
            ...
            int index = newPartsZipFile.FindEntry("Geometry/VX" + designID.PadLeft(7, '0') + "/m" + designID + ".fbx", true);
            if (index >= 0)
            {
                return true;
            }
        ...
        {
            if (File.Exists(Path.Combine(geometryPath, legacyDir, lod0Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, legacyDir, lightmappedDir, lod0Dir, designID + ".fbx")))
            {
                return true;
            }

            if (File.Exists(Path.Combine(geometryPath, legacyDir, lod1Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, legacyDir, lightmappedDir, lod1Dir, designID + ".fbx")))
            {
                return true;
            }

            if (File.Exists(Path.Combine(geometryPath, legacyDir, lod2Dir, designID + ".fbx")) || File.Exists(Path.Combine(geometryPath, legacyDir, lightmappedDir, lod2Dir, designID + ".fbx")))
            {
                return true;
            }
            ...
            int index = legacyPartsZipFile.FindEntry(designID + ".fbx", true);
            if (index >= 0)
            {
                return true;
            }
        ...
        static bool UnpackExactNewPart(string designID, bool lightmapped, int lod, bool forceUnpack = false)
        {
            ...
            int index = newPartsZipFile.FindEntry(path, true);
            if (index >= 0)
            {
                var entry = newPartsZipFile.GetEntry(path);
                if (entry.IsFile && entry.CanDecompress)
                {
                    ...
                    var zipStream = newPartsZipFile.GetInputStream(entry);
        ...
        static bool UnpackExactLegacyPart(string designID, bool lightmapped, int lod)
        {
            ...
            if (index >= 0)
            {
                var entry = legacyPartsZipFile.GetEntry(designID + ".fbx");
                if (entry.IsFile && entry.CanDecompress)
                {
                    ...
                    var zipStream = legacyPartsZipFile.GetInputStream(entry);
                    var fileStream = File.Create(filePath);
                    zipStream.CopyTo(fileStream);
                    fileStream.Dispose();
                    zipStream.Dispose();
        ...
        public static bool UnpackCommonPart(string name, bool lightmapped, int lod, bool forceUnpack = false)
        {
            ...
            if (lod <= 1)
            {
                ...
                int index = commonPartsZipFile.FindEntry(name + ".fbx", true);
                if (index >= 0)
                {
                    var entry = commonPartsZipFile.GetEntry(name + ".fbx");
                    if (entry.IsFile && entry.CanDecompress)
                    {
                        var fileName = entry.Name;
                        ...
                        var zipStream = commonPartsZipFile.GetInputStream(entry);
                        var fileStream = File.Create(filePath);
                        zipStream.CopyTo(fileStream);
                        fileStream.Dispose();
                        zipStream.Dispose();
            ...
            else
            {
                ...
                var knob = name.Contains("knob");
                var tube = name.Contains("tube");
                var duplo = name.StartsWith("D");
                var hollow = knob ? name.EndsWith("C") : tube;

                if (duplo)
                {
                    var tubeOrPinHeight = tube ? name.Contains("_01_") ? 0.6233f : name.Contains("_02_") ? 0.8f : name.Contains("_03_") ? 1.6f : 3.677f : 3.674f;
                    var height = knob ? 0.46f : tubeOrPinHeight;
                    var radius = knob ? 0.481f : tube ? 0.6765f : 0.3239f;
                    var innerRadius = radius - (tube ? 0.135f : 0.088f);
                    CreateCommonPartLod2(name, height, radius, knob, hollow, innerRadius, lightmapped);
                }
                else
                {
        ...
        public static bool CheckConnectivityForPart(string designID)
        {
            OpenDB();

            var partConnectivityPath = "CollisionBox_Connectivity_Info/" + designID + ".xml";
            int index = newPartsZipFile.FindEntry(partConnectivityPath, true);
        ...
        public static bool UnpackConnectivityForPart(string designID, bool forceUpdate = false)
        {
            ...
            if (index >= 0)
            {
                var entry = newPartsZipFile.GetEntry(partConnectivityPath);
                if (entry.IsFile && entry.CanDecompress)
                {
                    var zipStream = newPartsZipFile.GetInputStream(entry);
        ...
        public static bool UnpackCollidersForPart(string designID, bool forceUpdate = false)
        {
            ...
            int index = newPartsZipFile.FindEntry(partCollisionPath, true);
            if (index >= 0)
            {
                var entry = newPartsZipFile.GetEntry(partCollisionPath);
                if (entry.IsFile && entry.CanDecompress)
                {
                    var zipStream = newPartsZipFile.GetInputStream(entry);
        ...
        public static void CompareLegacyAndNewPartsDBs()
        {
            var legacyPartsZipFile = new ZipFile(legacyPartsPath);
            var entryEnumerator = legacyPartsZipFile.GetEnumerator();
            ...
            var newsPartsZipFile = new ZipFile(newPartsPath);
            entryEnumerator = newsPartsZipFile.GetEnumerator();
            ...
            {
                var entry = (ZipEntry)entryEnumerator.Current;
                if (entry.IsFile && entry.Name.StartsWith("Geometry") && entry.Name.EndsWith(".fbx"))
                {
        ...
        [MenuItem("LEGO Tools/Dev/Validate Connectivity For New Part DB")]
        public static void ValidateConnectivityForNewPartDB()
        {
            var newsPartsZipFile = new ZipFile(newPartsPath);
            var entryEnumerator = newsPartsZipFile.GetEnumerator();

            List<string> missingConnectivityIds = new List<string>();
            ...
            while (entryEnumerator.MoveNext())
            {
                ...
                if (entry.IsFile && entry.Name.StartsWith("Geometry") && entry.Name.EndsWith(".fbx"))
                {
                    ...
                    if (!CheckConnectivityForPart(designId))
                    {
                        missingConnectivityIds.Add(designId);
                    }
                }
            }
            newsPartsZipFile.Close();

            Debug.Log($"Missing connectivity count {missingConnectivityIds.Count}");

            Debug.Log("Parts with missing connectivity:");
            foreach (var missingConnectivity in missingConnectivityIds)
            {
                Debug.Log(missingConnectivity);
            }
        }
    }
}