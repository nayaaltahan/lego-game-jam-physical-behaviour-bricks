...
namespace LEGOModelImporter
{
    ...
    public static class ModelProcessor
    {
        ...
        static readonly int detailPixelThreshold = 20;
            ...
            foreach (var part in parts)
            {
                ...
                else
                {
                    ...
                    foreach (var renderer in part.GetComponentsInChildren<MeshRenderer>(true))
                    {
                        ...
                        else if (parentName == "Detail") // Is it an inside detail?
                        {
                            partMeshRenderer.type = MeshType.Detail;
                        }
                        ...
                        case 0:
                            {
                                ...
                                mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/LEGO_VertexColor_Fast.mat");
                                break;
                            }
                        case 1:
                            {
                                PartUtility.StoreOptimizedMesh(m, group.parentName + "_" + group.groupName + "_Optimized.asset");
                                mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/LEGO_VertexColor.mat");
                                break;
                            }
                        case 2:
                            {
                                PartUtility.StoreOptimizedMesh(m, group.parentName + "_" + group.groupName + "_Optimized_Transparent_Fast.asset");
                                mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/LEGO_VertexColor_Transparent_Fast.mat");
                                break;
                            }
                        case 3:
                            {
                                PartUtility.StoreOptimizedMesh(m, group.parentName + "_" + group.groupName + "_Optimized_Transparent.asset");
                                mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/LEGO_VertexColor_Transparent.mat");
                                break;
                            }
                    }
                }
        ...
        private static void AnalyzeMeshes(List<Matrix4x4> viewMatrices, List<Matrix4x4> projectionMatrices, List<Vector3> viewPositions, List<Vector3> viewDirections, List<Plane[]> viewFrustums, List<MeshInstance> instances, Dictionary<Part, int> partMeshCount, Dictionary<Part, int> partSurfaceMeshCount, bool allowInsideRemove, bool allowOutsideRemove, int lod)
        {
            ...
            Material material = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.lego.modelimporter/Materials/Optimizer.mat");
            ...
            if (allowInsideRemove || allowOutsideRemove)
            {
                foreach (var m in instances)
                {
                    ...
                    if (m.transparent)
                    {
                        var canRemove = ((m.type == MeshType.Legacy || m.type == MeshType.Shell || m.type == MeshType.ColourChange || m.type == MeshType.Knob) && allowOutsideRemove) || ((m.type == MeshType.Tube || m.type == MeshType.Detail) && allowInsideRemove);
                        if (canRemove)
                        {
                    ...
                    if (m.pixelCount > 0)
                    {
                        ...
                        else if (m.type == MeshType.Detail)
                        {
                            if (allowInsideRemove && m.pixelCount < detailPixelThreshold)
                            {
                                m.mesh = null;
                                partMeshCount[m.part]--;
                            }
                    ...
                    else
                    {
                        ...
                        else if ((m.type == MeshType.Tube || m.type == MeshType.Detail) && allowInsideRemove)
                        {
                            m.mesh = null;
                            partMeshCount[m.part]--;
                        }
