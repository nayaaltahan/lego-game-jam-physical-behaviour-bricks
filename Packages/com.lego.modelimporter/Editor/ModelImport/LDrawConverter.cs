...
namespace LEGOModelImporter
{
    ...
    public static class LDrawConverter
    {
        ...
        private static Dictionary<string, Assembly> LDrawAssembly;

        private static List<string> missingSubModels;
        private static List<string> missingAssemblies;
        private static List<string> incompleteAssemblies;
        ...
        public static XmlDocument ConvertLDrawToLXFML(Stream ldrawInput, string name)
        {
            ...
            incompleteAssemblies = new List<string>();
            ...
            var mappingText = File.ReadAllText("Packages/com.lego.modelimporter/Data/ldraw.xml");
            ...
            var assemblyMappingText = File.ReadAllText("Packages/com.lego.modelimporter/Data/ldrawassembly.xml");
            ...
            LDrawAssembly = new Dictionary<string, Assembly>();
            var assemblyNodes = assemblyMappingRoot.SelectNodes("Assembly");
            foreach (XmlNode assemblyNode in assemblyNodes)
            {
                var assembly = new Assembly();

                assembly.complete = XmlConvert.ToBoolean(assemblyNode.Attributes["complete"].Value);
                ...
                LDrawAssembly.Add(assemblyNode.Attributes["ldraw"].Value, assembly);
            }
        ...
        public static Dictionary<string, List<string>> GetErrors()
        {
            ...
            if (incompleteAssemblies.Count > 0)
            {
                errors.Add("The following multi-part bricks has missing parts. Replace with individual bricks, or add them to ldrawassembly.xml.", incompleteAssemblies);
            }
            ...
            if (line.Length > 0 && line[0] == '1')
            {
                ...
                if (tokens.Length == 15 && tokens[14].EndsWith(".dat", StringComparison.InvariantCulture))
                {
                    ...
                    if (assemblySplit.Length > 1)
                    {
                        ...
                        {
                            foreach (var part in LDrawAssembly[ldrawID].parts)
                            {
                                AddPartToBrick(brick, part, transformation, materialID, fullName);
                            }

                            if (!LDrawAssembly[ldrawID].complete)
                            {
                                incompleteAssemblies.Add(fullName);
                            }
                        }
                        else
                        {
