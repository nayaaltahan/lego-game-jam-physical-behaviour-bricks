...
namespace LEGOModelImporter
{
    public class ImportModel : EditorWindow
    {
        ...
        {
            var path = EditorUtility.OpenFilePanelWithFilters("Select model file", "Packages/com.lego.modelimporter/Models", new string[] { "All model files", "ldr,io,lxfml,lxf", "LDraw files", "ldr", "Studio files", "io", "LXFML files", "lxfml", "LXF files", "lxf" });
            if (path.Length != 0)
            {
        ...
        {
            EditorUtility.DisplayDialog("Failed to read model from file", "If you're reading an LXFML or LXF file, make sure that they are using version 5.6 or newer", "Ok");

        }

        private void OnEnable()
        {
            logoTexture = AssetDatabase.LoadAssetAtPath<Texture>("Packages/com.lego.modelimporter/Textures/LEGO logo.png");
        }
        ...
        private void OnGUI()
        {
            ...
            {
                // Show error message and copy-to-clipboard button.
                EditorGUI.HelpBox(new Rect(leftMargin, nextY, maxLabelWidth + 260.0f, 38.0f), trackedError.Key, MessageType.Warning);
                if (GUI.Button(new Rect(leftMargin + maxLabelWidth + 264.0f, nextY + 6.0f, 26.0f, 26.0f), EditorGUIUtility.IconContent("Clipboard", "|Copy to clipboard")))
                {
                    var clipboardContent = trackedError.Key + "\n";
        ...
        private static LXFMLDoc ReadFileLogic(string path)
        {
            ...
            switch (extension)
            {
                ...
                case ".lxf":
                    {
                        // Open LXF file.
                        var lxfFile = new ZipFile(path);

                        var entry = lxfFile.GetEntry("image100.lxfml");
                        if (entry.IsFile && entry.CanDecompress)
                        {
                            doc = new XmlDocument();
                            var lxfmlStream = lxfFile.GetInputStream(entry);
                ...
                case ".io":
                    {
                        // Open IO file.
                        var ioFile = new ZipFile(path);
                        ioFile.Password = "soho0909";

                        var entry = ioFile.GetEntry("model.ldr");
                        if (entry.IsFile && entry.CanDecompress)
                        {
                            var ldrStream = ioFile.GetInputStream(entry);
                            doc = LDrawConverter.ConvertLDrawToLXFML(ldrStream, path);
                            ldrStream.Dispose();
                            trackedErrors = LDrawConverter.GetErrors();
                        }
