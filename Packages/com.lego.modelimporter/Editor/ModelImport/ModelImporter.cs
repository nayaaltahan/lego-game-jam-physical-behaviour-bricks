    ...
    public class ModelImporter
    {
        ...
        public static GameObject InstantiateModelGroup(LXFMLDoc.BrickGroup group, int index, GameObject parent, string absoluteFilePath, string relativeFilePath, ref Dictionary<int, Brick> resultBricks, bool isSubGroup, ModelGroupImportSettings importSettings)
        {
            ...
            if (!isSubGroup)
            {
                ...
                groupComp.viewBasedOptimizations = ModelGroup.ViewBasedOptimizations.None;
            }
        ...
        public static void InstantiateModelBricks(LXFMLDoc lxfml, DictionaryIntToModelGroupImportSettings importSettings, ref Dictionary<int, Brick> resultBricks, int groupNumber = -1, bool undo = false)
        {
            for (var i = 0; i < lxfml.bricks.Count; ++i)
            {
                ...
                foreach (var part in brick.parts)
                {
                    ...
                    partComp.partType = part.partType;
                    if (part.decorations != null && part.decorations.Length > 0)
                    {
                        partComp.decorations = part.decorations.Select(decoration => decoration.ToString()).ToList();
                    }
                    partComp.designID = Convert.ToInt32(partExistenceResult.designID);
                    partComp.stickerSheetID = part.stickerSheetId;
                    partComp.variantID = part.variantId;
        ...
        public static void ReimportModel(LXFMLDoc lxfml, Model model, Model.Pivot previousPivot, DictionaryIntToModelGroupImportSettings importSettings)
        {
            ...
            if(model.pivot != Model.Pivot.Original)
            {
                ModelGroupUtility.RecomputePivot(model);
            }

            var oldBounds = ComputeBounds(model.transform);
            ...
            var newBounds = ComputeBounds(model.transform);

            if (model.pivot == Model.Pivot.Original && previousPivot != Model.Pivot.Original)
            {
                model.transform.position += oldBounds.center - newBounds.center;
            }
        ...
        public static void ReimportModelGroup(LXFMLDoc lxfml, ModelGroup group, ModelGroupImportSettings importSettings, bool detectConnectivity = false)
        {
            ...
            group.transform.SetParent(originalGroupParent, true);
